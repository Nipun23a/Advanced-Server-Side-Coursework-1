...
class BidModel {
    static async create(bidData){
        ...
        const [result] = await pool.execute(
            `INSERT INTO bids (user_id, bid_amount, bid_status, bid_date, is_cancelled, sponsorship_total, created_at, updated_at)
    ...
    static async findLatestActiveBid(userId){
        ...
        const [rows] = await pool.execute(
            `SELECT id, bid_amount, bid_date FROM bids 
    ...
    static async getHighestBidForDate(bidDate) {
        ...
        const [rows] = await pool.execute(
            `SELECT MAX(bid_amount) AS highest_amount FROM bids 
    ...
    static async findCandidatesForDate(bidDate) {
        ...
        const [rows] = await pool.execute(
            `SELECT id, user_id, bid_amount, created_at FROM bids 
             WHERE bid_date = ? AND bid_status = 'active' AND is_cancelled = false
    ...
    static async updateAmount(bidId, newAmount) {
        ...
        const [result] = await pool.execute(
            'UPDATE bids SET bid_amount = ?, updated_at = NOW() WHERE id = ?',
            [newAmount, bidId]
        );
    ...
    static async cancel(bidId) {
        const [result] = await pool.execute(
            'UPDATE bids SET is_cancelled = true, updated_at = NOW() WHERE id = ?',
            [bidId]
        );
        return result.affectedRows > 0;
    }
    ...
    static async getHistoryByUser(userId, page = 1, limit = 10) {
        ...
        const [bids] = await pool.execute(
            `SELECT id, bid_amount, bid_status, bid_date, is_cancelled, created_at, updated_at
             FROM bids WHERE user_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?`,
            [userId, limit, offset]
        );
