...
class BiddingController {
    static async placeBid(req, res) {
        try {
            const { bid_amount, bid_date, user_id, sponsorship_offer_id } = req.body;
            ...
            const result = await BiddingService.placeBid(
                ...
                bid_date,
                sponsorship_offer_id ? parseInt(sponsorship_offer_id, 10) : null
            );
    ...
    static async updateBid(req, res) {
        try {
            ...
            const { bid_amount, user_id, sponsorship_offer_id } = req.body;
            const userId = user_id || req.internalUserId;
            ...
            const result = await BiddingService.updateBid(
                bidId,
                userId,
                parseFloat(bid_amount),
                sponsorship_offer_id ? parseInt(sponsorship_offer_id, 10) : null
    ...
    static async markEventAttendance(req, res) {
        try {
            ...
            const result = await BiddingService.markEventAttendance(userId, eventName, eventDate);
